...
export interface ButtonProps
  ...
  size?: "sm" | "md" | "lg";
  /** Icon for icon-only buttons (no children required) */
  icon?: IconName;
  leftIcon?: IconName;
  rightIcon?: IconName;
...
const variantStyles = {
  ...
  ghost:
    "text-chalkboard-foreground hover:bg-chalkboard-background focus:ring-chalkboard-primary",
};
...
export function Button({
  ...
  const paddingStyles = isIconOnly ? iconOnlySizeStyles[size] : sizeStyles[size];

  const classes = `${baseStyles} ${variantStyles[variant]} ${paddingStyles} ${className}`.trim();
